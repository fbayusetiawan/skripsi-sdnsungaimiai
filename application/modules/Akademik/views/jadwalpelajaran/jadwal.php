<?php
$linkin = $this->uri->segment(1) . '/' . $this->uri->segment(2);
$no = '1';
?>
<div class="row page-title">
    <div class="col-md-12">
        <nav aria-label="breadcrumb" class="float-right mt-1">
            <ol class="breadcrumb">
                <li><a href="<?= base_url($linkin . '/add') ?>" class="btn btn-success">Tambah Data</a></li>

            </ol>
        </nav>
        <h4 class="mb-1 mt-0">Data <?= $title ?></h4>
    </div>
</div>
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <!-- <form action="<?= base_url($linkin . '/viia/') ?>" method="post" class="needs-validation" novalidate>
                    <div class="form-group mb-3">
                        <label for="validationCustom01">Pilih Kelas</label>
                        <select name="kodeKelas" id="kelas" required class="form-control">
                            <option value="">SILAHKAN PILIH KELAS</option>
                            <?php foreach ($kelas as $kelas) : ?>
                                <option value="<?= $kelas->kodeKelas ?>"><?= $kelas->namaKelas ?> </option>
                            <?php endforeach ?>
                        </select>
                    </div>



                    <button class="btn btn-primary" type="submit">Submit</button>
                     <a href="<?= base_url($linkin) ?>" class="btn btn-danger">Kembali</a> 
                </form> -->
                <table id="basic-datatable" class="table dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Hari</th>
                            <th>Kelas</th>
                            <th>Jam Mulai / Selesai</th>
                            <th>Nama Pelajaran</th>
                            <th>Nama Guru</th>
                            <th>Aksi</th>

                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($data as $row) : ?>
                            <tr>
                                <td><?= $no++ ?></td>
                                <td><?= $row->namaHari ?></td>
                                <td><?= $row->namaKelas ?></td>
                                <td><?= $row->jamMulai ?> / <?= $row->jamSelesai ?></td>
                                <td><?= $row->namaMapel ?></td>
                                <td><?= $row->namaGuru ?></td>
                                <td>
                                    <div class="btn-group mb-0">
                                        <!-- <a href="<?= base_url($linkin . '/viia') ?>" class="btn btn-info btn-sm" data-toggle="tooltip" title="Detail"><i class="uil uil-search-alt"></i></a> -->
                                        <a href="<?= base_url($linkin . '/edit/' . $row->kodeJadwal) ?>" class="btn btn-info btn-sm" data-toggle="tooltip" title="Edit"><i class="uil uil-edit"></i></a>
                                        <a href="<?= base_url($linkin . '/delete/' . $row->kodeJadwal) ?>" id="<?= $row->namaMapel ?>" class="delete-data btn btn-info btn-sm" data-toggle="tooltip" title="Hapus"><i class="uil uil-trash-alt"></i></a>
                                    </div>
                                </td>

                            </tr>
                        <?php endforeach ?>
                    </tbody>
                </table>
            </div> <!-- end card body-->
        </div> <!-- end card -->
    </div><!-- end col-->
</div>
<!-- end row-->